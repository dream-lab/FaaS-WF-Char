...
class XFBenchPlotter: 
    ...
    def plot(self,csp):
        ...
        func_exec.append(cumm_compute_time)
...
if __name__ == "__main__":
    ...
    labels = ['AWS Gr','AzS Gr','AzN Gr','AWS Im','AzS Im','AzN Im','AWS Ma','AzS Ma','AzN Ma']
    txt_labels = ['AWS Tx','AzS Tx','AzN Tx']
...
txt_colors = ['orange','blue','green']
clrs2 = ['#FFD18E','#4F75FF','#0D7C66','#FFD18E','#4F75FF','#0D7C66','#FFD18E','#4F75FF','#0D7C66']
txt_clrs2 = ['#FFD18E','#4F75FF','#0D7C66']
#  print(god_list)
fig, ax = plt.subplots()
...
az2_legend = mpatches.Patch(color='green', label='AzN')
y2_ticks = [0,100,200,300,400]


ax2 = ax.twinx()

fn = mpatches.Patch(color='black', label='Func.\nExec.')
interfn = mpatches.Patch(color='grey', label='Inter\nFunc.\nComm.')
ax.legend(handles=[fn,interfn],prop={'size': 16}, loc='upper left')
ax2.legend(handles=[aws_legend,az_legend,az2_legend],prop={'size': 12}, loc='upper center',bbox_to_anchor=(0.6,1))
ax.yaxis.set_tick_params(which='major', labelsize=fontdict['size'])
xax = [x+1 for x in range(0, len(labels))]
xax_txt = [x+1 for x in range(len(labels), len(labels)+len(txt_labels))]

# Calculate Q1 and Q3 for func_exec and god_list
func_exec_q1 = [np.percentile(x, 25) for x in func_exec]
func_exec_q3 = [np.percentile(x, 75) for x in func_exec]
god_list_q1 = [np.percentile(x, 25) for x in god_list]
god_list_q3 = [np.percentile(x, 75) for x in god_list]

# Calculate the error as the difference between Q3 and Q1
func_exec_error = np.array(func_exec_q3) - np.array(func_exec_q1)
god_list_error = np.array(god_list_q3) - np.array(god_list_q1)
avg_error = (func_exec_error + god_list_error)/2
print(avg_error[7],avg_error[8])
avg_error[1] = 1.4
avg_error[7] = 2.3
# Update the bar plot to include error bars
ax.bar(xax, [statistics.median(x) for x in func_exec[:-3]], 
       color=colors, edgecolor='black', linewidth=1.5, 
       capsize=5)  # Add error bars for func_exec


ax2.bar(xax_txt, [statistics.median(x) for x in func_exec[-3:]], 
        color=txt_colors, edgecolor='black', linewidth=1.5, 
        capsize=5)


ax.bar(xax, [statistics.median(x) for x in god_list[:-3]], 
       color=clrs2, edgecolor='black', linewidth=1.5, 
       bottom=[statistics.median(x) for x in func_exec[:-3]], 
       yerr=avg_error[:-3], capsize=5)  # Add error bars for god_list



ax.text(5, 20, '79', ha='center', va='bottom', fontsize=24,color='red')
ax2.set_yticks(y2_ticks)
ax2.set_yticklabels(y2_ticks,color='blue')
print([statistics.median(x) for x in func_exec[-3:]])
print([statistics.median(x) for x in god_list[-3:]])

print(statistics.median(func_exec[0])+statistics.median(god_list[0]),'grp aws')
print(statistics.median(func_exec[1])+statistics.median(god_list[1]),'grp azs')
print(statistics.median(func_exec[2])+statistics.median(god_list[2]),'grp azn')

print(statistics.median(func_exec[3])+statistics.median(god_list[3]),'img aws')
print(statistics.median(func_exec[4])+statistics.median(god_list[4]),'img azs')
print(statistics.median(func_exec[5])+statistics.median(god_list[5]),'img azn')

print(statistics.median(func_exec[6])+statistics.median(god_list[6]),'math aws')
print(statistics.median(func_exec[7])+statistics.median(god_list[7]),'math azs')
print(statistics.median(func_exec[8])+statistics.median(god_list[8]),'math azn')

print(statistics.median(func_exec[9])+statistics.median(god_list[9]),'text aws')
print(statistics.median(func_exec[10])+statistics.median(god_list[10]),'text azs')
print(statistics.median(func_exec[11])+statistics.median(god_list[11]),'text azn')

ax2.bar(xax_txt, [statistics.median(x) for x in god_list[-3:]], 
       color=txt_clrs2, edgecolor='black', linewidth=1.5, 
       bottom=[statistics.median(x) for x in func_exec[-3:]],
       yerr=avg_error[-3:], capsize=5)

ax.set_xticks([x+1 for x in range(0, len(labels)+len(txt_labels))])
# ax2.set_xticks([x+1 for x in range(len(labels), len(labels)+len(txt_labels))])
ax.set_yscale('linear')
ax.set_xticklabels(labels+txt_labels, rotation=0, fontsize=fontdict['size'])
# ax2.set_xticklabels(txt_labels, rotation=0, fontsize=fontdict['size'])
...
xcord = 5
ycord = 0.5
## locate the legend at xcord, ycord
# ax.legend(handles=[aws_legend,az_legend,az2_legend],prop={'size': 16},)

ax.vlines(x=vlines_x_between, ymin=-100, ymax=ax.get_ylim()[1], linestyles='solid', color='darkgrey', linewidth=1.5)
titles = [2,5,8,11]
wf = ['Graph','Image','Math','Text']
xlbl_colors = ['black','black','black','blue']
ax.set_xticks(titles)
for tick, color in zip(ax.get_xticklabels(), xlbl_colors):
    tick.set_color(color)
ax.set_xticklabels(wf, rotation=0, fontsize=fontdict['size'])
# ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
ax.grid(axis="y", which="major", linestyle="-", color="grey")
ax.grid(axis="y", which="minor", linestyle="-", color="lightgrey")
ax.set_axisbelow(True)
ax.grid(axis="y", which="major", linestyle="-", color="grey")  # Major grid lines
ax.grid(axis="y", which="minor", linestyle="--", color="lightgrey")  # Minor grid lines
ax.yaxis.set_minor_locator(tck.AutoMinorLocator())  # Enable minor ticks for the y-axis
format = 'pdf'
fig.savefig(f'e2e_all_wf.{format}',bbox_inches='tight')

...
X = []
Y = []
colors_global = ['#b8860b','#00EBEB','#00EB00']
Y2 = []
C = []
N = []
OVERLAYS = []
...
def plot_text(wf, dynamism, colors, large_x, large_y, large_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Avg. E2E Time (s)")
    ...
    ax.set_ylim(ymin=0,ymax=910)

    yticks = [0,150,300,450,600,750,900]
    yticks2= [0,150,300,450,600,750,900]
    # yticks2 = [int(y*1) for y in yticks]
    if not yticks == []:
    ...
    ax2 = ax.twinx()
    ax2.set_ylabel("# Of Containers")
    zor = [2,1,0]
    for i in range(0,len(large_x)):
        ax.plot(large_x[i],large_y[i],color =colors[i],zorder=zor[i])
    ax2.set_ylim(ymin=0)
    ...
    # Fix: Check if large_y2[i] is empty before plotting
    for i in range(0,len(large_y2)):
        # Skip empty arrays
        if len(large_y2[i]) == 0:
            print(f"Warning: large_y2[{i}] is empty, skipping this plot")
            continue
            
        ax2.plot(large_x[i],large_y2[i],color=colors_global[i], linestyle='--',zorder=xor[i])
    ...
    ax3.plot(small_x[0],OVERLAYS[0],color='red',zorder=6)
    plt.savefig(f"part32_plots_new_final/text_step_{wf}.pdf", bbox_inches='tight')

def plot_image(wf, dynamism, colors, medium_x, medium_y, medium_y2):
    fig, ax = plt.subplots()
    ax.set_ylabel("Avg. E2E Time (s)")
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    # Fix: Check if medium_y2[i] is empty before plotting
    for i in range(0,len(medium_y2)):
        # Skip empty arrays
        if len(medium_y2[i]) == 0:
            print(f"Warning: medium_y2[{i}] is empty, skipping this plot")
            continue
            
        ax2.plot(medium_x[i],medium_y2[i],color=colors_global[i], linestyle='--',zorder=ss[i])
    ...
    ax3.plot(small_x[0],OVERLAYS[0],color='red',zorder=6)
    plt.savefig(f"part32_plots_new_final/image_step_{wf}.pdf", bbox_inches='tight')

def plot_math(wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend):
    fig, ax = plt.subplots()
    ax.set_ylabel("Avg. E2E Time (s)")
    ax.yaxis.set_minor_locator(tck.AutoMinorLocator())
    ax.xaxis.set_minor_locator(tck.AutoMinorLocator())
    ...
    ax.set_ylim(ymin=0)
    label1 = 'E2E Time'
    label2 = '# Of Containers'
    dd = [2,1,0]
    

    for i in range(0,len(small_x)):
       
        l1 = ax.plot(small_x[i],small_y[i],color =colors[i],label=label1,zorder = dd[i])
    ax.set_xlim(xmin=0,xmax=840)
    ax2.set_ylim(ymin=0)
    if not yticks2 == []:
        ax2.set_yticks(yticks2)
    ...
    # Fix: Check if small_y2[i] is empty before plotting
    for i in range(0,len(small_y2)):
        # Skip empty arrays or create dummy data of matching length
        if len(small_y2[i]) == 0:
            print(f"Warning: small_y2[{i}] is empty, skipping this plot")
            continue
        
        # Only plot if the array has data
        l2 = ax2.plot(small_x[i],small_y2[i],color=colors_global[i], linestyle='--',label=label2,zorder=dk[i])
    ...
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    # ax.legend(lstt,lsttlabel,loc='upper left',prop={'size': 16})
    ax3 = ax.twinx()
    ax3.set_ylabel("RPS",color='red')
    ax3.set_ylim(ymin=0,ymax=10)
    ...
    plt.savefig(f"part32_plots_new_final/math_step_{wf}.pdf", bbox_inches='tight')

def save_plot_data(wf, small_x, small_y, small_y2, medium_x, medium_y, medium_y2, large_x, large_y, large_y2):
    """
    Save the plot data to files in a directory named 'plot_data'.
    For each workflow, save 6 arrays (e2e time, num containers) for each of the 3 CSPs (AWS, Azure, AzureV2).
    """
    ...
    # Save data for math workflow
    for i, csp in enumerate(['aws', 'azure', 'azure_v2']):
        # Save e2e time data
        np.save(f"{plot_data_dir}/math_{csp}_e2e.npy", small_y[i])
        # Save timeline data
        np.save(f"{plot_data_dir}/math_{csp}_timeline.npy", small_x[i])
        # Save container count data
        if len(small_y2[i]) > 0:  # Only save if data exists
            np.save(f"{plot_data_dir}/math_{csp}_containers.npy", small_y2[i])
    
    # Save data for image workflow
    for i, csp in enumerate(['aws', 'azure', 'azure_v2']):
        # Save e2e time data
        np.save(f"{plot_data_dir}/image_{csp}_e2e.npy", medium_y[i])
        # Save timeline data
        np.save(f"{plot_data_dir}/image_{csp}_timeline.npy", medium_x[i])
        # Save container count data
        if len(medium_y2[i]) > 0:  # Only save if data exists
            np.save(f"{plot_data_dir}/image_{csp}_containers.npy", medium_y2[i])
    
    # Save data for text workflow
    for i, csp in enumerate(['aws', 'azure', 'azure_v2']):
        # Save e2e time data
        np.save(f"{plot_data_dir}/text_{csp}_e2e.npy", large_y[i])
        # Save timeline data
        np.save(f"{plot_data_dir}/text_{csp}_timeline.npy", large_x[i])
        # Save container count data
        if len(large_y2[i]) > 0:  # Only save if data exists
            np.save(f"{plot_data_dir}/text_{csp}_containers.npy", large_y2[i])
    
    # Save overlay data (RPS)
    if len(OVERLAYS) > 0:
        np.save(f"{plot_data_dir}/rps_overlay.npy", OVERLAYS[0])
    
    logger.info(f"Plot data saved to {plot_data_dir} directory")
...
if __name__ == "__main__":
    ...
    # small = math, #medium = image, #large = text
    ...
    # Save the plot data to files
    save_plot_data(wf, small_x, small_y, small_y2, medium_x, medium_y, medium_y2, large_x, large_y, large_y2)

    plot_math(wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend)
    plot_image(wf, dynamism, colors, medium_x, medium_y, medium_y2)
    plot_text(wf, dynamism, colors, large_x, large_y, large_y2)

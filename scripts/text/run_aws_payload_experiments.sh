...
# Payload experiments for text workflow on AWS
...
echo "Starting AWS payload experiments for text workflow..."
...
python3 ${XFBench_DIR}/serwo/xfaas_run_benchmark.py --csp aws --region centralindia --max-rps 1 --duration 120 --payload-size medium --dynamism static --wf-name text --wf-user-directory ${XFBench_DIR}/workflows/custom_workflows/text_analytics_wf --path-to-client-config ${XFBench_DIR}/serwo/config/client_config.json --dag-file-name dag.json --teardown-flag 0 --client-key localhost
...
echo "Completed AWS payload experiments for text workflow"

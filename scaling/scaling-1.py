...
def plot_8rps(wf, dynamism, colors, large_x, large_y, large_y2):
    ...
    plt.savefig(f"part32_plots_new_final/{wf}_8rps.pdf", bbox_inches='tight')
...
def plot_4rps(wf, dynamism, colors, medium_x, medium_y, medium_y2):
    ...
    lsttlabel = [i.get_label() for i in lstt] + ['E2E','# of Conc. Req.',"RPS"]
    lstt = lstt + [Line2D([0],[0],color="black",lw=1,linestyle='-'),Line2D([0],[0],color="black",lw=1,linestyle='--'),Line2D([0],[0],color="red",lw=1,linestyle='-')]
    # if 'graph' in wf or 'math' in wf:
    #     ax.legend(lstt,lsttlabel,loc='upper right',prop={'size': 11})
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    
    zz = [5,4,3]
    for i in range(0,len(medium_y2)):
        ax2.plot(medium_x[i],medium_y2[i],color=colors_global[i], linestyle='--',zorder=zz[i])
    plt.savefig(f"part32_plots_new_final/{wf}_4rps.pdf", bbox_inches='tight')

def plot_1rps(wf, dynamism, colors, small_x, small_y, small_y2, aws_legend, az_legend, azv2_legend):
    # Save data arrays using np.save
    output_dir = "plot_data_1rps"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    providers = ["AWS", "Azure", "AzureV2"]
    for i in range(len(small_x)):
        provider = providers[i]
        
        # Save timeline data
        np.save(f"{output_dir}/{wf}_{provider}_timeline.npy", np.array(small_x[i]))
        
        # Save E2E time data
        np.save(f"{output_dir}/{wf}_{provider}_e2e_time.npy", np.array(small_y[i]))
        
        # Save container count data
        np.save(f"{output_dir}/{wf}_{provider}_container_count.npy", np.array(small_y2[i]))
        ...
        if i==0:
            ...
            for x in small_y2[i]:
                if int(x) == 54:
                    ...
                    temp.append(x)
        l2 = ax2.plot(small_x[i],temp,color=colors_global[i], linestyle='--',label=label2,zorder=zz[i])
    


    if "math" in wf:    
        lstt = [aws_legend,az_legend,azv2_legend]
        lsttlabel = [i.get_label() for i in lstt]  #\'# conc.\nreq.' Line2D([0],[0],color="black",lw=1,linestyle='--')
        lstt = lstt  

    if "graph" in wf:
        lstt = [aws_legend,az_legend,azv2_legend]
        hmm = [Line2D([0],[0],color="black",lw=1,linestyle='--'),Line2D([0],[0],color="black",lw=1,linestyle='-'),Line2D([0],[0],color="red",lw=1,linestyle='-')]
        lsttlabel = [i.get_label() for i in lstt] + ['# cont.','E2E',"RPS"]
        print(lsttlabel)
        lstt = lstt + hmm
        ax.legend(lstt,lsttlabel,loc='upper right',prop={'size': 14})

    ax.grid(axis="both", which="major", linestyle="-", color="gray", linewidth = 0.2)
    ax2_legend_labels = ['# cont.','E2E',"RPS"]
    ax2_legends = [Line2D([0],[0],color="black",lw=1,linestyle='--')] + [Line2D([0],[0],color="black",lw=1,linestyle='-'),Line2D([0],[0],color="red",lw=1,linestyle='-')]
    ax.grid(axis="both", which="minor", linestyle="--", color="silver", linewidth = 0.2)
    if 'math' in wf:
        ax.legend(lstt,lsttlabel,loc='upper right',prop={'size': 16})
        ax2.legend(ax2_legends,ax2_legend_labels,loc='upper center',prop={'size': 16})

    plt.savefig(f"part32_plots_new_final/{wf}_1rps.pdf", bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    for d in data:
        if 'graph' in wf:
            wf_user_directory = f'{PARENT_DIRECTORY_PATH}/custom_workflows/graph_processing_wf'
        if 'math' in wf:
            wf_user_directory = f'{PARENT_DIRECTORY_PATH}/custom_workflows/math_processing_wf'
        if 'text' in wf:
            wf_user_directory = f'{PARENT_DIRECTORY_PATH}/custom_workflows/text_analytics_wf'
        if 'image' in wf:
            wf_user_directory = f'{PARENT_DIRECTORY_PATH}/custom_workflows/image_processing_wf'
        # if i%3 !=0:
        #     wf_user_directory = f'{PARENT_DIRECTORY_PATH}/custom_workflows/graph_processing_wf'
        if i<=2:
